  ...
  for (const [clientName, itemsMap] of clientMap.entries()) {
    ...
    html += `<div class="invoice-client-card">
      ...
      <table class="invoice-items-table">
        ...
        <tbody>${sortedItems.map(([itemName, qty]) => `<tr><td>${escapeHtml(itemName)}</th><td style="text-align:right;font-weight:600">${qty}</th>`).join('')}</tbody>
      </table>
    </div>`;
  }
    ...
    for (const [clientName, itemsMap] of clientMap.entries()) {
      ...
      bodyHtml += `<div style="border:1px solid #e2d5c0;margin-bottom:20px;page-break-inside:avoid;">
        ...
        <table style="width:100%;border-collapse:collapse;">
          ...
          <tbody>${sortedItems.map(([itemName, qty]) => `<tr><td style="padding:8px 18px;border-bottom:1px solid #e2d5c0;">${escapeHtml(itemName)}</th><td style="padding:8px 18px;text-align:right;font-weight:600;">${qty}</th>`).join('')}</tbody>
        </table>
      </div>`;
    }
