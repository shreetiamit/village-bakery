...
function printTab(tab) {
  const printStyles = `
    * { margin: 0; padding: 0; box-sizing: border-box; }
    ...
    .date-header-left { font-size: 11px; font-weight: 600; text-transform: uppercase; }
    ...
    .item-clients { font-size: 9px; color: #6b6860; }
    .item-qty { font-size: 13px; font-weight: 700; text-align: right; }
    .item-unit { font-size: 9px; font-weight: 400; }
    .client-card { border: 1px solid #dedad4; margin-bottom: 10px; page-break-inside: avoid; }
    .client-header { background: #f8f6f2; padding: 5px 8px; display: flex; justify-content: space-between; border-bottom: 1px solid #dedad4; font-size: 10px; font-weight: 600; }
    ...
    .invoice-client-card { border: 1px solid #dedad4; margin-bottom: 12px; page-break-inside: avoid; }
    .invoice-client-header { background: #f5efdf; padding: 6px 10px; border-bottom: 1px solid #dedad4; }
    .invoice-client-name { font-size: 13px; font-weight: 700; }
    .invoice-client-stats { font-size: 9px; color: #6b6860; }
    .invoice-items-table th { padding: 5px 10px; font-size: 9px; background: #f8f6f2; }
    .invoice-items-table td { padding: 5px 10px; font-size: 10px; border-bottom: 1px solid #f0ece6; }
  ...
  if (tab === 'invoicing') {
    ...
    for (const [clientName, itemsMap] of clientMap.entries()) {
      const sortedItems = Array.from(itemsMap.entries()).sort((a, b) => a[0].localeCompare(b[0]));
      const totalUnits = sortedItems.reduce((s, i) => s + i[1], 0);
      bodyHtml += `<div class="invoice-client-card">
        <div class="invoice-client-header">
          ...
          <div class="invoice-client-stats">${sortedItems.length} item(s) · ${totalUnits} units</div>
        </div>
    ...
    win.document.write(`<!DOCTYPE html><html><head><meta charset="UTF-8"><title>Invoicing Summary</title><style>${printStyles}</style></head><body>
      ...
      <div style="display:flex; gap: 16px; margin-bottom: 20px;">
        <div><strong>${clientMap.size}</strong> Clients</div>
    ...
    for (const date of sortedDates) {
      ...
      for (const vendor of sortedVendors) {
        ...
        body += `<div class="client-card">
          ...
          ${data.notes ? `<div style="padding: 4px 8px; font-size: 9px; font-style: italic; border-top: 1px dashed #dedad4;">Note: ${escapeHtml(data.notes)}</div>` : ''}
        </div>`;
      }
      body += `</div>`;
    }
